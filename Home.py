...
grouped_claims_by_week = (
    claims.groupby(by=["Year", "Week"]).agg(Claimed=("Claimed", "sum")).reset_index()
)
grouped_claims_by_week["label"] = (
    "Week "
    + grouped_claims_by_week["Week"].astype(str).str.zfill(2)
    + " "
    + grouped_claims_by_week["Year"].astype(str)
)
grouped_claims_by_month = (
    claims.groupby(by=["Year", "Month", "Month Name"])
    .agg(Claimed=("Claimed", "sum"))
    .reset_index()
)
grouped_claims_by_month["label"] = (
    grouped_claims_by_month["Year"].astype(str)
    + " "
    + grouped_claims_by_month["Month"].astype(str).str.zfill(2)
)
...
time_frames = {
    ...
    "2024": {
        "data": claims[
            claims["DateofAttendance"].between("2024-02-15", "2024-08-31")
        ].copy(),
    },
}
...
if claim_period:
    ...
    data["Types"] = data["Types"].where(
        ~np.logical_and(data["Types"].eq("Others"), data["Inout"].eq("In")),
        "In-Patient Services",
    )  # type: ignore
    data["Types"] = data["Types"].where(
        ~np.logical_and(data["Types"].eq("Others"), data["Inout"].eq("Out")),
    ...
    facility_types["percent_value"] = (
        facility_types["Claimed"] / facility_types["Claimed"].sum()
    ) * 100
    facility_types["Percentage"] = (
        facility_types["percent_value"].round(2).astype(str) + "%"
    )
    facility_types = facility_types.sort_values(
        by=["Claimed"], ascending=False
    ...
    grouped_data_specific = grouped_data[
        ~grouped_data["ServiceType"].isin(
            ["Universal Service", "Diagnostic Center", "Pharmacy"]
        )
    ].copy()
    ...
    facilities = facilities.sort_values(by=["cost"], ascending=False).reset_index(
        drop=True
    )
    ...
    cost_per_service = (
        data.groupby(by=["TypeName"]).agg(cost=("Claimed", "sum")).reset_index()
    )  # type: ignore
    cost_per_service = cost_per_service.sort_values(
        by=["cost"], ascending=False
    ).reset_index(drop=True)
    cost_per_service["percent_weight"] = (
        cost_per_service["cost"] / cost_per_service["cost"].sum()
    )
    cost_per_service["percent_value"] = cost_per_service["percent_weight"]
    cost_per_service["Percentage"] = (
        cost_per_service["percent_value"].round(2).astype(str) + "%"
    )
    top_5_services = cost_per_service.head(5)

    claims_of_top_5_services = data[
        np.logical_and(
            data["TypeName"].isin(top_5_services["TypeName"]),
            data["ServiceProvider"].isin(facilities["ServiceProvider"]),
        )
    ].copy()  # type: ignore
    summarized_top_5_services_per_facility = (
        claims_of_top_5_services.groupby(
    ...
    top_5_service_with_top_5_facilities = summarized_top_5_services_per_facility[
    ...
    claims_for_top_3_facility_types = data[
        data["ServiceType"].isin(top_3_facility_types["ServiceType"])
    ].copy()  # type: ignore
    summary_for_top_3_facility_types = (
        claims_for_top_3_facility_types.groupby(by=["ServiceProvider", "ServiceType"])
        .agg(cost=("Claimed", "sum"))
        .reset_index()
    )
    ...
    for facility_type in top_3_facility_types["ServiceType"]:
        all_facilities_with_type = summary_for_top_3_facility_types[
            summary_for_top_3_facility_types["ServiceType"] == facility_type
        ].copy()
        top_facilities_with_type = (
            all_facilities_with_type.sort_values(by=["cost"], ascending=False)
            .reset_index(drop=True)
            .head(5)
        )
        claims_of_top_5_services_from_top_facilities = data[
            np.logical_and(
                data["TypeName"].isin(top_5_services["TypeName"]),
                data["ServiceProvider"].isin(
                    top_facilities_with_type["ServiceProvider"]
                ),
            )
        ].copy()  # type: ignore
        ...
        top_facilities_claims_with_top_service_types = pd.concat(
            [
...
if claim_period:
    ...
    st.metric(
        "Average cost per visit:", "{:,.2f}".format(round(average_cost_per_visit, 1))
    )


st.markdown(
    "### What are the percentage cost associated with each Facility Type and each Service type?"
)
...
for facility_type in top_3_facility_types["ServiceType"]:
    top = top_facilities_claims_with_top_service_types[
        top_facilities_claims_with_top_service_types["ServiceType"] == facility_type
    ].copy()
    top_grouped = (
        top.groupby(by=["ServiceProvider"]).agg(cost=("cost", "sum")).reset_index()
    )
        ...
        if number_check > 4:
            ...
            if facility_type != top_facility_type:
                ...
                if chart:
                    ...
                    top_facility_type = top_facilities_claims_with_top_service_types[
                        top_facilities_claims_with_top_service_types["ServiceType"]
                        == top_facility_type
                    ].copy()
                    top_facility_type_with_current_service = top_facility_type[
                        top_facility_type["TypeName"].isin(
                            service_types_in_current_facility_type
                        )
                    ]
            ...
            final = pd.merge(
                final,
                facilities[["ServiceProvider", "rank"]],
                how="left",
                on="ServiceProvider",
            )
            final = final.sort_values(by=["rank"], ascending=True).reset_index(
                drop=True
            )
            ...
            for service_type in final["ServiceType"].unique():
                ...
                for type_name in service_type_df["TypeName"].unique():
                    type_name_df = service_type_df[
                        service_type_df["TypeName"] == type_name
                    ]
